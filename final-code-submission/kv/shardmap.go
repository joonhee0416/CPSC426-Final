...
func (sm *ShardMap) NodesNotForShard(shard int) []string {
	nodesForShard := sm.NodesForShard(shard)

	state := sm.GetState()
	nodes := state.Nodes

	nodesNotForShard := make([]string, 0)

	for node := range nodes {
		if containsNode(nodesForShard, node) {
			continue
		}
		nodesNotForShard = append(nodesNotForShard, node)
	}
	return nodesNotForShard
}
